  ...
  if (!courses.length) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
        ...
        {emptyAction && (
          <Link
            className="mt-5 inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-bold text-white hover:bg-emerald-800"
    ...
    <div className="grid gap-5 md:grid-cols-2">
      {courses.map((course) => (
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" key={course.id}>
          ...
          <Link
            className="mt-5 inline-flex rounded-lg border border-emerald-700 px-4 py-2 text-sm font-bold text-emerald-800 hover:bg-emerald-50"
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-sm font-bold uppercase tracking-wider text-emerald-700">Teacher workspace</p>
            <h1 className="mt-2 text-3xl font-black text-slate-950">My Courses</h1>
          </div>
          <Link className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-bold text-white hover:bg-emerald-800" to="/teacher/courses/create">
        ...
        {error && <div className="mt-8 rounded-2xl border border-red-200 bg-red-50 p-6 text-red-800"><p>{error}</p><button className="mt-3 font-bold underline" onClick={reload} type="button">Try again</button></div>}
        {!loading && !error && <div className="mt-8"><CourseCards courses={courses} emptyAction /></div>}
      </section>
    </main>
  );
}

...
function youtubeVideoId(value) {
  try {
    ...
    return host === "youtu.be" ? url.pathname.slice(1) : url.searchParams.get("v");
  } catch {
    return null;
  }
}
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 break-words">
        <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">{type}</p>
  ...
  return (
    <section className="mt-10 border-t border-slate-200 pt-7" aria-labelledby="supplementary-materials-heading">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h2 className="text-lg font-black text-slate-950" id="supplementary-materials-heading">Supplementary Learning Materials</h2>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {documents.map((material) => {
            ...
            return <MaterialCard key={material.id} tone={isPdf ? "pdf" : "document"}>
              <MaterialHeading Icon={FileText} title={material.title} type={isPdf ? "PDF document" : "Document"} />
      ...
      {videos.length > 0 && <div className="mt-5 space-y-4">
        {videos.map((material) => {
          const videoId = youtubeVideoId(material.external_url);
          return <MaterialCard key={material.id} tone="video">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              {videoId && <button aria-label={`Open ${material.title}`} className="ph-video-thumbnail group relative aspect-video w-full shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100 sm:w-[200px]" onClick={() => openExternal(material.external_url)} type="button">
                <img alt="" className="h-full w-full object-cover transition-transform duration-200 group-hover:scale-105" loading="lazy" src={`https://img.youtube.com/vi/${videoId}/hqdefault.jpg`} />
                <span aria-hidden="true" className="absolute inset-0 grid place-items-center bg-slate-950/10"><span className="grid size-11 place-items-center rounded-full bg-red-600 text-white shadow-md"><Play fill="currentColor" size={18} /></span></span>
              </button>}
              <div className="min-w-0 flex-1"><MaterialHeading Icon={Film} title={material.title} type="Video" />{material.description && <p className="mt-3 text-sm leading-6 text-slate-600">{material.description}</p>}</div>
              <button className="ph-action inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-emerald-700 px-3 py-2.5 text-sm font-bold text-white hover:bg-emerald-800" onClick={() => openExternal(material.external_url)} type="button">Watch Video <ExternalLink aria-hidden="true" size={15} /></button>
            </div>
          </MaterialCard>;
        })}
      </div>}
      ...
      {links.length > 0 && <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {links.map((material) => <MaterialCard key={material.id} tone="external">
          <MaterialHeading Icon={ExternalLink} title={material.title} type="External resource" />
      ...
      {forms.length > 0 && <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {forms.map((material) => <MaterialCard key={material.id} tone="form">
          <MaterialHeading Icon={FileQuestion} title={material.title} type="Google Form" />
          {material.description && <p className="mt-3 text-sm leading-6 text-slate-600">{material.description}</p>}

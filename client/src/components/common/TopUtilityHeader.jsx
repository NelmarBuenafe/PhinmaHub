...
export default function TopUtilityHeader({ role, drawerOpen, drawerId, onOpenDrawer }) {
  ...
  const context = pathname === base ? "Dashboard"
    ...
    : pathname.startsWith(`${base}/courses/`) ? (role === "Student" ? "Course Learning" : "Course Overview")
    : pathname === `${base}/assignments` ? "Assignments"
    : pathname === `${base}/announcements` ? "Announcements"
    : pathname === `${base}/join-course` ? "Join Course" : "My Profile";

  return (
    <header className="ph-utility-header sticky top-0 z-30 flex h-[60px] min-w-0 items-center gap-2 border-b border-slate-200/80 bg-white px-4 md:gap-3 md:px-6 min-[1200px]:px-7">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-1.5">
        ...
        <AccountMenu onSignOut={signOut} profile={profile} profileRoute={`${base}/profile`} role={role} />
      </div>
    </header>
  );
}
